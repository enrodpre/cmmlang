...
# optionally include cotire - the git submodule might not be inited (or the user
# might have already included it)
...
option(
  UCM_UNITY_BUILD
  "Enable unity build for targets registered with the ucm_add_target() macro"
  OFF)
option(
  UCM_NO_COTIRE_FOLDER
  "Do not use a cotire folder in the solution explorer for all unity and cotire related targets"
  ON)

# ucm_add_flags Adds compiler flags to CMAKE_<LANG>_FLAGS or to a specific
# config
macro(ucm_add_flags)
...
# ucm_set_flags Sets the CMAKE_<LANG>_FLAGS compiler flags or for a specific
# config
macro(ucm_set_flags)
  ucm_add_flags(CLEAR_OLD ${ARGN})
endmacro()

# ucm_remove_flags Removes compiler flags from CMAKE_<LANG>_FLAGS or from a
# specific config
...
# ucm_add_linker_flags Adds linker flags to CMAKE_<TYPE>_LINKER_FLAGS or to a
# specific config
macro(ucm_add_linker_flags)
  cmake_parse_arguments(ARG "CLEAR_OLD;EXE;MODULE;SHARED;STATIC" "" "CONFIG"
                        ${ARGN})
...
# ucm_set_linker_flags Sets the CMAKE_<TYPE>_LINKER_FLAGS linker flags or for a
# specific config
macro(ucm_set_linker_flags)
  ucm_add_linker_flags(CLEAR_OLD ${ARGN})
endmacro()

# ucm_gather_flags Gathers all lists of flags for printing or manipulation
macro(ucm_gather_flags with_linker result)
  ...
  if("${CMAKE_CONFIGURATION_TYPES}" STREQUAL "" AND NOT "${CMAKE_BUILD_TYPE}"
                                                    STREQUAL "")
    # handle single config generators - like makefiles/ninja - when
...
# ucm_set_runtime Sets the runtime (static/dynamic) for msvc/gcc
macro(ucm_set_runtime)
  ...
  # add/replace the flags note that if the user has messed with the flags
  # directly this function might fail - for example if with MSVC and the user
  # has removed the flags - here we just switch/replace them
  if("${ARG_STATIC}")
    foreach(flags ${flags_configs})
    ...
    foreach(flags ${flags_configs})
      if(CMAKE_CXX_COMPILER_ID MATCHES "GNU")
        if(${flags} MATCHES "-static-libstdc\\+\\+")
          string(REGEX REPLACE "-static-libstdc\\+\\+" "" ${flags}
                               "${${flags}}")
        endif()
...
# ucm_print_flags Prints all compiler flags for all configurations
...
# ucm_set_xcode_attrib Set xcode attributes - name value CONFIG config1
# conifg2..
...
# ucm_count_sources Counts the number of source files
macro(ucm_count_sources)
  cmake_parse_arguments(ARG "" "RESULT" "" ${ARGN})
  if(${ARG_RESULT} STREQUAL "")
    message(
      FATAL_ERROR
        "Need to pass RESULT and a variable name to ucm_count_sources()")
  endif()
...
# ucm_include_file_in_sources Includes the file to the source with compiler
# flags
macro(ucm_include_file_in_sources)
  ...
  if(${ARG_HEADER} STREQUAL "")
    message(
      FATAL_ERROR
        "Need to pass HEADER and a header file to ucm_include_file_in_sources()"
    )
  endif()
...
# ucm_dir_list Returns a list of subdirectories for a given directory
...
# ucm_trim_front_words Trims X times the front word from a string separated with
# "/" and removes the front "/" characters after that (used for filters for
# visual studio)
...
# ucm_remove_files Removes source files from a list of sources (path is the
# relative path for it to be found)
macro(ucm_remove_files)
  cmake_parse_arguments(ARG "" "FROM" "" ${ARGN})

  if("${ARG_UNPARSED_ARGUMENTS}" STREQUAL "")
    message(
      FATAL_ERROR "Need to pass some relative files to ucm_remove_files()")
  endif()
  if(${ARG_FROM} STREQUAL "")
    message(
      FATAL_ERROR "Need to pass FROM and a variable name to ucm_remove_files()")
  endif()
...
# ucm_remove_directories Removes all source files from the given directories
...
macro(ucm_remove_directories)
  ...
  if("${ARG_UNPARSED_ARGUMENTS}" STREQUAL "")
    message(
      FATAL_ERROR
        "Need to pass some relative directories to ucm_remove_directories()")
  endif()
  if(${ARG_FROM} STREQUAL "")
    message(
      FATAL_ERROR
        "Need to pass FROM and a variable name to ucm_remove_directories()")
  endif()
...
macro(ucm_add_files_impl result trim files)
  foreach(cur_file ${files})
    ...
    # replacing forward slashes with back slashes so filters can be generated
    # (back slash used in parsing...)
    string(REPLACE "/" "\\" FILTERS "${FILEPATH}")
    source_group("${FILTERS}" FILES ${cur_file})
  endforeach()
endmacro()

# ucm_add_files Adds files to a list of sources
macro(ucm_add_files)
  ...
  if(${ARG_TO} STREQUAL "")
    message(
      FATAL_ERROR "Need to pass TO and a variable name to ucm_add_files()")
  endif()
...
macro(ucm_add_dir_impl result rec trim dirs_in additional_ext)
  ...
  foreach(cur_dir ${dirs})
    ...
    # since unix is case sensitive - add these valid extensions too we don't use
    # "UNIX" but instead "CMAKE_HOST_UNIX" because we might be cross compiling
    # (for example emscripten) under windows and UNIX may be set to 1 Also OSX
    # is case insensitive like windows...
    set(additional_file_extensions "")
    if(CMAKE_HOST_UNIX AND NOT APPLE)
      set(additional_file_extensions "${cur_dir}*.CPP" "${cur_dir}*.C"
                                     "${cur_dir}*.H" "${cur_dir}*.HPP")
    endif()
    ...
    file(
      ...
      # https://gcc.gnu.org/onlinedocs/gcc-4.4.1/gcc/Overall-Options.html
      # #index-file-name-suffix-71 sources
      "${cur_dir}*.cpp"
      "${cur_dir}*.cxx"
      "${cur_dir}*.c++"
      "${cur_dir}*.cc"
...
# ucm_add_dirs Adds all files from directories traversing them recursively to a
# list of sources and generates filters according to their location (accepts
# relative paths only). Also this macro trims X times the front word from the
# filter string for visual studio filters.
macro(ucm_add_dirs)
  cmake_parse_arguments(ARG "RECURSIVE" "TO;FILTER_POP" "ADDITIONAL_EXT"
                        ${ARGN})
  ...
  ucm_add_dir_impl(${ARG_TO} ${ARG_RECURSIVE} ${ARG_FILTER_POP}
                   "${ARG_UNPARSED_ARGUMENTS}" "${ARG_ADDITIONAL_EXT}")
endmacro()

# ucm_add_target Adds a target eligible for cotiring - unity build and/or
# precompiled header
macro(ucm_add_target)
  cmake_parse_arguments(ARG "UNITY" "NAME;TYPE;PCH_FILE;CPP_PER_UNITY"
                        "UNITY_EXCLUDED;SOURCES" ${ARGN})
  ...
  if("${ARG_NAME}" STREQUAL "")
    message(
      FATAL_ERROR
        "Need to pass NAME and a name for the target to ucm_add_target()")
  endif()
  ...
  if(${is_type_valid} STREQUAL "-1")
    message(
      FATAL_ERROR
        "Need to pass TYPE and the type for the target [EXECUTABLE/STATIC/SHARED/MODULE] to ucm_add_target()"
    )
  endif()
  if("${ARG_SOURCES}" STREQUAL "")
    message(
      FATAL_ERROR
        "Need to pass SOURCES and a list of source files to ucm_add_target()")
  endif()
  ...
  # unity build only for targets with > 1 source file (otherwise there will be
  # an additional unnecessary target)
  ...
  # inform the developer that the current target might benefit from a unity
  # build
  ...
  if(do_unity)
    ...
    foreach(excluded_file "${ARG_UNITY_EXCLUDED}")
      set_source_files_properties(${excluded_file} PROPERTIES COTIRE_EXCLUDED
                                                              TRUE)
    endforeach()
  endif()
  ...
  if(do_unity)
    ...
    if(NOT "${ARG_CPP_PER_UNITY}" STREQUAL "")
      ...
      set_property(
        TARGET ${orig_target}
        PROPERTY COTIRE_UNITY_SOURCE_MAXIMUM_NUMBER_OF_INCLUDES "100")
    endif()

    if(NOT "${ARG_PCH_FILE}" STREQUAL "")
      set_target_properties(
        ${orig_target} PROPERTIES COTIRE_CXX_PREFIX_HEADER_INIT
                                  "${ARG_PCH_FILE}")
    else()
      set_target_properties(${orig_target}
                            PROPERTIES COTIRE_ENABLE_PRECOMPILED_HEADER FALSE)
    endif()
    # add a unity target for the original one with the name intended for the
    # original
    set_target_properties(${orig_target} PROPERTIES COTIRE_UNITY_TARGET_NAME
                                                    ${ARG_NAME})
    ...
    set_target_properties(
      ${orig_target} PROPERTIES EXCLUDE_FROM_ALL 1 EXCLUDE_FROM_DEFAULT_BUILD 1)
    set_target_properties(
      ${unity_target_name} PROPERTIES EXCLUDE_FROM_ALL 0
                                      EXCLUDE_FROM_DEFAULT_BUILD 0)

    # also set the name of the target output as the original one
    set_target_properties(${unity_target_name} PROPERTIES OUTPUT_NAME
                                                          ${ARG_NAME})
    if(UCM_NO_COTIRE_FOLDER)
      # reset the folder property so all unity targets dont end up in a single
      # folder in the solution explorer of VS
  ...
  elseif(NOT "${ARG_PCH_FILE}" STREQUAL "")
    ...
    if(ucm_with_cotire)
      set_target_properties(${orig_target} PROPERTIES COTIRE_ADD_UNITY_BUILD
                                                      FALSE)
      set_target_properties(
        ${orig_target} PROPERTIES COTIRE_CXX_PREFIX_HEADER_INIT
                                  "${ARG_PCH_FILE}")
  ...
  if(wanted_cotire AND NOT ucm_with_cotire)
    if(NOT COMMAND cotire)
      message(
        AUTHOR_WARNING
          "Target \"${ARG_NAME}\" not cotired because cotire isn't loaded")
    else()
      message(
        AUTHOR_WARNING
          "Target \"${ARG_NAME}\" not cotired because cotire is older than the required version"
      )
    endif()
  endif()
endmacro()
