...
void Preprocessor::process_includes() {
  ...
  while (next_pos != std::string::npos) {
    ...
    size_t next_nl = find_next_newline(remaining);
    auto line      = remaining.substr(next_nl);
    svmatch include_match;
    bool success = match_regex_sv(line, INCLUDE_PATTERN, include_match);
    if (!success || include_match.empty()) {
      /* throw exception::preprocessor_error("Include directives must go in a
       * line itself"); */
    }
...
void Preprocessor::process_defines() {
  ...
  while (next_pos != std::string::npos) {
    ...
    auto line      = remaining.substr(next_nl);
    svmatch include_match;
    bool ready = match_regex_sv(line, DEFINE_PATTERN, include_match);
    if (ready && include_match.ready() && !include_match.empty()) {
      auto name = include_match[1];
