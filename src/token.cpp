...
[[nodiscard]] const std::vector<token>& tokens::data() const noexcept {
  return m_tokens;
}
...
bool tokens::has_next() const noexcept {
  return pointer() < size();
}
...
void tokens::advance(size_t n) {
  m_pointer += n;
}

RCtoken tokens::peek(short offset) {
  return m_tokens.at(m_pointer + offset);
}

size_t tokens::pointer() const noexcept {
  return m_pointer;
}
